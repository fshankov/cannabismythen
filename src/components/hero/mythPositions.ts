 ...
 * canvas position and a fallback display label. At runtime the homepage
 * supplies the **real** myth statement (derived from the .mdoc title via
 * `deriveHeroLabel` in `@lib/myth`); the `text` field below is kept only
 * as a safety fallback when the page can't resolve a real title (e.g.
 * during a fresh install before content syncs).
 *
 * Position units: x/y are percentages of the hero canvas (0–100).
 * Canvas reference: ~1400px wide × ~820px tall (desktop).
 *
 * Long real titles are handled at render time by adaptive font scaling
 * inside `HeroBlock.tsx` — we don't need to repack positions per title.
 */
export interface MythPosition {
  ...
  size: number; // base font-size in px (auto-scaled if real title is long)
  serif: boolean;
  weight: number;
  italic: boolean;
}
...
export const mythPositions: MythPosition[] = [
  ...
  { id: "m05",     text: "Cannabis ist schwer dosierbar",        x:  5, y: 29, r:  1, size: 22, serif: false, weight: 400, italic: false },
  ...
  { id: "m25",     text: "Cannabis wirkt gegen Angst",           x: 57, y: 57, r: -1, size: 17, serif: false, weight: 400, italic: false },
