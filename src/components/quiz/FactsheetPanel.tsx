/**
 * FactsheetPanel — Slide-in panel showing a myth's full factsheet.
 *
 * Layout mirrors the Zahlen & Fakten page:
...
import type { MythContentEntry } from "./QuizPlayer";
import { t } from "./i18n";
...
export default function FactsheetPanel({ myth, mythContentEntry, onClose, statementText, explanationText }: FactsheetPanelProps) {
  ...
  const renderSection = (section: Section, i: number) => {
    ...
    if (isCollapsible) {
      const countLabel = section.title === "Referenzen"
        ...
        <div key={i} className="factsheet-panel__section factsheet-panel__section--collapsible">
          ...
          >
            <span className="factsheet-panel__section-title">
              {section.title}
              {countLabel && (
                <span className="factsheet-panel__section-count">
                  ({countLabel})
                </span>
              )}
            </span>
            <span className={`factsheet-panel__chevron ${isExpanded ? "factsheet-panel__chevron--open" : ""}`}>
              &#9662;
            </span>
          </button>
          {isExpanded && (
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t(myth.statementKey)}
    >
      <div
        ref={panelRef}
        className="factsheet-panel"
        tabIndex={-1}
      >
        ...
        <div className="factsheet-panel__body">
          {mythContentEntry ? (
            <>
              {/* 1. Myth statement */}
              <p className="factsheet-panel__statement">
                {mythContentEntry.title}
              </p>

              {/* 2. Evidence-based verdict */}
              <div className="factsheet-panel__classification">
                <span className="factsheet-panel__label">
                  {t("ui.correctAnswer")}
                </span>
                <span
            ...
            <>
              ...
              <div className="factsheet-panel__section">
                <div className="factsheet-panel__section-content">
                  <p>{explanationText || t(myth.explanationKey)}</p>
                </div>
              </div>
