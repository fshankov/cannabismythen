...
import SourcesSpannweiteView, {
  type SourcesSpannweiteViewHandle,
} from "./views/SourcesSpannweiteView";
import SourcesTableView, {
  type SourcesTableViewHandle,
} from "./views/SourcesTableView";
...
export default function MythenExplorer({
  mythSlugs,
  mythContent,
  definitions,
  mythThemes,
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    merged.groupIds = correctGroupsForIndicator(
      merged.indicator,
      merged.groupIds,
    );
    return merged;
  ...
  const activeFilterCount = useMemo(() => {
    ...
    const categoryMythIds = new Set(
      data.myths
        .filter(
          (m) =>
            m.category_id !== null && state.categoryIds.includes(m.category_id),
        )
        .map((m) => m.id),
    );
  ...
  const SOURCES_VIEWS = new Set<ViewTab>([
    "sources",
    "sources2",
    "sources_table",
  ]);
  ...
  const mythSlugMap = useMemo(() => {
    ...
    return new Map(
      Object.entries(mythSlugs).map(([id, slug]) => [Number(id), slug]),
    );
  }, [mythSlugs]);
  ...
  useEffect(() => {
    const corrected = correctGroupsForIndicator(
      state.indicator,
      state.groupIds,
    );
  ...
  useEffect(() => {
    try {
      setRundgangSeen(
        window.localStorage.getItem("carm-rundgang-seen-v1") === "1",
      );
    } catch {
  ...
  const searchValue = isSourceView
    ? state.sourcesSearchQuery
    : state.searchQuery;
  const searchKey: keyof AppState = isSourceView
    ? "sourcesSearchQuery"
    : "searchQuery";
  ...
  const sharedActions: ReactNode = (
    <>
      <div className="carm-myth-search-row" role="search">
        <Search
          size={16}
          className="carm-myth-search-icon"
          aria-hidden="true"
        />
        <input
          type="search"
        ...
        <div className="carm-explorer__tab-bar">
          ...
          <div className="carm-dataset-toggle">
            <PivotToggle
              options={[
                {
                  ...
                  info: (
                    <InfoTooltip
                      ...
                      titleSuffix={
                        <span
                          className="carm-datasetinfo-verdicts"
                          aria-hidden="true"
                        >
                          <VerdictArrow
                            verdict="richtig"
                      ...
                      definition={
                        <>
                          ...
                          <p className="info-tooltip-foot">
                            {t("datasetInfo.mythen.foot", "de")}
                          </p>
                        </>
                      }
                    />
                    ...
                    <InfoTooltip
                      ...
                      titlePrefix={
                        <Signpost
                          size={15}
                          strokeWidth={2}
                          className="carm-datasetinfo-signpost"
                          aria-hidden="true"
                        />
                      }
                      definition={
              ...
              onChange={(ds) =>
                handleDatasetSwitch(ds as "mythen" | "informationswege")
              }
        ...
        <div className="carm-explorer__panel">
          {state.view === "balken" &&
            (() => {
              ...
              // (v4, 2026-05-26).
              const indicatorOptions: DataPickerOption<Indicator>[] =
                INDICATORS.map((ind) => {
              ...
              const groupOptions: DataPickerOption<GroupId>[] = GROUPS.map(
                (g) => {
          ...
          {!isModernView &&
            ...
            state.view !== "spannweite" && (
              <VerdictTags
                ...
                onChange={(f: VerdictFilter) => update("verdictFilter", f)}
              />
            )}
