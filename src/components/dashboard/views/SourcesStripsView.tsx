...
const SourcesStripsView = forwardRef<SourcesStripsViewHandle, Props>(
  function SourcesStripsView(
    ...
    const [sourceData, setSourceData] = useState<InformationSourcesData | null>(
      null,
    );
    useEffect(() => {
    ...
    const visibleColumnIds: string[] = allColumnIds.filter(
      (id) => !isHidden(id),
    );
    ...
    const columns = useMemo<ColumnData[]>(() => {
       ...
       */
      const widthBase =
        width < 480 ? 5 : width < 768 ? 7 : width < 1280 ? 9 : 11;
      ...
      const buildNodes = (
        ...
        const sim = d3
          ...
          .force(
            ...
            d3.forceCollide<BeeswarmNode>(
              (d) => (d.isChild ? radius * 0.7 : radius) + 0.6,
            ),
          )
          .stop();
      ...
      const metricData =
        sourceData.metrics[selectedMetric]?.data ||
        ({} as Record<SourceGroupId, Record<string, number>>);
    ...
    const hoveredSource: InformationSource | null = useMemo(() => {
      if (
        ...
        hoveredSourceId === selectedSourceId
      )
        return null;
    ...
    const metricRow: RowItem[] = useMemo(
      () =>
        METRICS.map((m) => {
          const def = definitions?.sourcesIndicators?.[m];
          return {
    ...
    const selectedCategory: SourceCategory | undefined =
      selectedSource && sourceData
        ? sourceData.sourceCategories.find(
            (c) => c.id === selectedSource.category,
          )
        : undefined;
    // (`selectedHasChildren` removed in Stage 4 — the "Unterkategorien
    //  zeigen" button it gated is gone now that subcategories always
    //  render.)
    const selectedParent: InformationSource | null =
      selectedSource && sourceData && selectedSource.parentId !== null
        ? (sourceData.sources.find((s) => s.id === selectedSource.parentId) ??
          null)
        : null;
    ...
    const hoveredParent: InformationSource | null =
      hoveredSource && sourceData && hoveredSource.parentId !== null
        ? (sourceData.sources.find((s) => s.id === hoveredSource.parentId) ??
          null)
        : null;
    ...
    return (
      <div className="strips-v3" ref={containerRef}>
        <div
          className={`strips-grid3 ${selectedSource ? "has-selection" : ""}`}
        >
          {/*
        ...
        */}
          <div className="strips-grid3__content" ref={contentRef}>
            ...
            {(() => {
              const activeId =
                topRow.items.find((it) => it.active)?.id ??
                topRow.items[0]?.id ??
                "";
              const valuePickerOptions: DataPickerOption<string>[] =
                        ...
                        if (next === ALL) {
                          ...
                          if (subFilter.length > 0 && sourceData) {
                            const stillValid = subFilter.filter((id) => {
                              const src = sourceData.sources.find(
                                (s) => s.id === id,
                              );
                          ...
                          d={(() => {
                            ...
                            columns.forEach((col, i) => {
                              const node = col.nodes.find(
                                (n) => n.sourceId === focusSource.id,
                              );
                              if (!node) return;
                              pts.push({
                                x: colCenterX(i) + node.x,
                                y: node.y,
                              });
                            });
                      ...
                      return (
                        ...
                        >
                          {sortedNodes.map((n) => {
                            ...
                            const isHov =
                              n.sourceId === hoveredSourceId && !isSel;
                            const dimmed =
                              focusSourceId !== null && !(isSel || isHov);
                            const baseR = n.isChild
                              ? col.radius * 0.7
                              : col.radius;
                            const r = isSel
                              ? baseR + 2.5
                            ...
                            const dotY = n.y;
                            const fill =
                              categoryColorMap.get(n.source.category) ||
                              "#6B7280";
                    ...
                    {focusSource && (
                      ...
                      >
                        {columns.map((col, i) => {
                          ...
                          const v =
                            col.valueBySource.get(focusSource.id) ?? null;
                  ...
                  {hoveredSource && hoverPos && !selectedSource && (
                    <div
                      ...
                      style={{
                        left: Math.max(
                          8,
                          Math.min(width - 260, hoverPos.x - 130),
                        ),
                ...
                {selectedSource && (
                  <div className="strips-myth-card" role="status">
                    ...
                    <div
                      ...
                      style={{
                        background: selectedCategory?.color || "#6B7280",
                      }}
                    >
                    ...
                    <p className="strips-myth-card__statement">
                      {cleanName(selectedSource.name)}
                    </p>
                    {selectedParent && (
                      <p className="strips-myth-card__summary">
                        Unterkategorie von{" "}
                        <strong>{cleanName(selectedParent.name)}</strong>
                      </p>
                    )}
                  ...
                  axis of the chart each number maps to. */}
                    <div className="strips-myth-card__values">
                      {columns.map((col) => {
                        const v =
                          col.valueBySource.get(selectedSource.id) ?? null;
                        return (
                          <div
                            key={`val-${String(col.id)}`}
                            className="strips-myth-card__value"
                          >
                            <span className="strips-myth-card__value-label">
                              {col.shortLabel}
                            </span>
                            <span className="strips-myth-card__value-num">
                              {formatPillValue(v)}
                            </span>
                          </div>
                        );
