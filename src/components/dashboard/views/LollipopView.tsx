...
export default function LollipopView({
  myths,
  metrics,
  groups,
  state,
  ...
  const option = useMemo(() => {
    const yCategories = groups.map((g) =>
    ...
    if (selectedMythId !== null) {
      ...
      for (let i = 0; i < selectedPoints.length - 1; i++) {
        markLineData.push([
          {
            coord: [
              selectedPoints[i].value,
              selectedPoints[i].groupIndex + jitter(selectedMythId),
            ],
          },
          {
            coord: [
              selectedPoints[i + 1].value,
              selectedPoints[i + 1].groupIndex + jitter(selectedMythId),
            ],
          },
        ]);
      }
