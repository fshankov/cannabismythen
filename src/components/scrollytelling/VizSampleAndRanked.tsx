...
const SAMPLE_GROUPS: readonly SampleGroup[] = [
  {
    id: "adults",
    label: "Erwachsene 18–70 Jahre",
    n: 2097,
    iconCount: 210,
    color: "var(--group-adults)",
    Icon: IconVolljaehrige,
  },
  {
    id: "minors",
    label: "Minderjährige 16–17 Jahre",
    n: 555,
    iconCount: 56,
    color: "var(--group-minors)",
    Icon: IconMinderjaehrige,
  },
  {
    id: "clubs",
...
const INDICATORS: readonly IndicatorMeta[] = [
  {
    name: "Kenntnis",
    unit: "Anteil %",
    desc: "Wer hat die Aussage überhaupt schon gehört?",
    mini: "Wer hat die Aussage überhaupt schon gehört?",
    key: "awareness",
    phase: "awareness",
  },
  {
    name: "Bedeutung",
    unit: "Punkte",
    desc: "Wie stark prägt die Aussage den eigenen Umgang mit Cannabis?",
    mini: "Wie stark prägt die Aussage den eigenen Umgang?",
    key: "significance",
    phase: "significance",
  },
  {
    name: "Richtigkeit",
    unit: "Punkte 0–100",
    desc: "Wie nahe liegt die Einschätzung an der wissenschaftlichen Klassifikation?",
    mini: "Wie nahe liegt die Einschätzung an der Wissenschaft?",
    key: "correctness",
    phase: "correctness",
  },
  {
    ...
    unit: "Punkte",
    desc: "Bedeutung × Wissenslücke — wo Aufklärung am meisten Wirkung zeigt.",
    mini: "Bedeutung × Wissenslücke — wo Aufklärung wirkt.",
    key: "prevention_significance",
    phase: "prevention_significance",
  },
  {
    name: "Bevölkerungsrelevanz",
    unit: "Punkte",
    desc: "Präventionsbedeutung × Kenntnisanteil — nur für Voll- und Minderjährige.",
    mini: "Präventionsbedeutung × Bekanntheit — nur Voll- + Minderjährige.",
    key: "population_relevance",
    phase: "population_relevance",
  },
];
...
const BEV_RISIKO_VALID_GROUPS: ReadonlySet<GroupId> = new Set<GroupId>([
  "adults",
  "minors",
]);
...
export function VizSampleAndRanked({ data, mode }: Props) {
  ...
  const exampleMyths = EXAMPLE_MYTH_IDS.map((id) =>
    data.myths.find((m) => m.id === id),
  ...
  return (
    <div className="viz viz-sr" data-mode={mode}>
      ...
      <div className="viz-sr__panels-stack">
        ...
        <div
          className="viz-sr__sample"
          aria-hidden={!isSample}
              ...
              {SAMPLE_GROUPS.map((g, gi) => (
                ...
                >
                  <div className="viz-sr__group-header">
                    <span className="viz-sr__group-num">
                      {g.n.toLocaleString("de-DE")}
                    </span>
...
function ExampleMythStrips({
  ...
  const valueMatrix = myths.map((myth) => {
    ...
    return {
      ...
      values: INDICATORS.map((ind) => {
        if (!m) return null;
        if (ind.key === "population_relevance" && !bevRisikoApplies)
          return null;
        return m[ind.key];
      }),
    };
  ...
  const hoveredMyth =
    hoveredId !== null ? (myths.find((m) => m.id === hoveredId) ?? null) : null;
    ...
    <div className="viz-sr__example" {...hoverHandlers}>
      <div className="viz-sr__example-header">
        ...
        <p className="viz-sr__example-hint">
          Fünf Mythen mit unterschiedlichem Aufklärungsbedarf. Berühre eine
          Linie, um die These zu lesen.
        </p>
      </div>
      ...
      <div className="viz-sr__strips-wrap" style={{ position: "relative" }}>
        ...
        >
          ...
          {INDICATORS.map((ind, i) => {
            ...
            const bevRisikoDimmed =
              isBevRisikoStrip && !BEV_RISIKO_VALID_GROUPS.has(activeGroup);
          ...
          {mythPoints.map(({ myth, points }) => {
            ...
            return points.slice(0, -1).map((p, segIdx) => {
              ...
              const segRevealIdx = isNewlyRevealed
                ? laterIdx - prevRevealed
                : 0;
              ...
              return (
                <line
                  ...
                  style={segStyle}
                  onMouseEnter={(e) =>
                    openMythTooltip(myth.id, e.currentTarget)
                  }
          ...
          {INDICATORS.map((ind, i) => {
            ...
            const bevRisikoDimmed =
              isBevRisikoStrip && !BEV_RISIKO_VALID_GROUPS.has(activeGroup);
            const settledOpacity = bevRisikoDimmed ? 0.55 : 1;
            const unitText = bevRisikoDimmed
              ? "nur Voll- + Minderjährige"
              : ind.unit;
          ...
          {mythPoints.map(({ myth, points }) =>
            points.map((p, i) => {
              ...
              const settledOpacity = stripIsRevealed ? (isDimmed ? 0.3 : 1) : 0;
              const stripFill =
                CORRECTNESS_PATH_COLOR[myth.correctness_class] ??
                "var(--fg-muted)";
