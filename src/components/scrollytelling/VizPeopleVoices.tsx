...
const VOICE_POOL: ReadonlyArray<[VoiceGroup, string]> = [
  ...
  [
    "parents",
    "Schädigt das Hirn in der Entwicklung bei Kindern und Jugendlichen",
  ],
...
function buildReadable(): ReadableDef[] {
  ...
  for (let i = 0; i < VOICE_POOL.length; i++) {
    ...
    out.push({
      ...
      group,
      text,
      side: TAIL_SIDES[i % TAIL_SIDES.length],
    });
    ...
    function project() {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        el.style.zIndex = String(
          funneling ? Math.round(500 + a * 200) : Math.round(a * 1000),
        );
        el.style.opacity = fop.toFixed(2);
        if (
          el.style.filter === "none" ||
          lastBlur[i] === undefined ||
          Math.abs(lastBlur[i] - bl) > 0.2
        ) {
      ...
      sphere.style.setProperty(
        "--fit",
        funneling ? "1" : (maxExtent > m ? m / maxExtent : 1).toFixed(3),
      );
    }
    ...
    function onOver(ev: Event) {
      ...
      if (hovered) {
        ...
        prev.style.transition =
          "transform 150ms ease-out, opacity 150ms ease-out, background 150ms ease, border-color 150ms ease, box-shadow 150ms ease";
        prev.style.transform = prev.style.transform.replace(
          /scale\([\d.]+\)/,
          "scale(" + ps.toFixed(3) + ")",
        );
      ...
      t.style.transition =
        "transform 200ms cubic-bezier(0.22,1,0.36,1), opacity 200ms ease, background 200ms ease, border-color 200ms ease, box-shadow 200ms ease";
