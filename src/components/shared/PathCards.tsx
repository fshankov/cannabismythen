...
const TILES: Tile[] = [
  {
    ...
    title: "42 Mythen",
    description:
      "Alle 42 Mythen auf Karten zum Umdrehen: vorn der Mythos, hinten die wissenschaftliche Einordnung.",
    url: withBase("fakten-karten/"),
  },
  {
    mod: "quiz",
    title: "Quiz",
    description:
      "6 Quiz, thematisch sortiert — teste dein Wissen in wenigen Minuten.",
    url: withBase("quiz/"),
  },
  {
    mod: "meine",
    ...
    description:
      "Fragen und Fakten, aufbereitet für 5 Zielgruppen: Eltern, Jugendliche, Konsumierende, Lehrkräfte und Fachkräfte.",
    url: withBase("meine-interessen/eltern/"),
  },
  {
    mod: "daten",
    title: "Daten-Explorer",
    description:
      "Wer glaubt was? Welche Informationswege werden genutzt? — bis zu 5 Indikatoren für 5 Zielgruppen im Vergleich.",
    url: withBase("daten-explorer/"),
  },
];
...
const FAKTEN_SPECTRUM: CorrectnessClass[] = [
  "falsch",
  "eher_falsch",
  "eher_richtig",
  "richtig",
  ...
  useEffect(() => {
    ...
    const tween = (toHover: boolean) => {
      ...
      const step = (now: number) => {
        ...
        bars.forEach((b, i) => {
          b.cur = lerp(starts[i], targets[i], e);
          apply(b, b.cur);
        });
      ...
      {TILES.map((c, i) => (
        ...
        >
          <span className="path-card__go" aria-hidden="true">
            ...
            <svg
              className="path-card__go-chevron"
              viewBox="0 0 24 24"
              width="16"
              height="16"
              fill="none"
          ...
          <span className="path-card__icon" aria-hidden="true">
            <svg
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              dangerouslySetInnerHTML={{ __html: ICONS[c.mod] }}
            />
          </span>
          ...
          {c.mod === "quiz" && (
            <div className="quiz-preview" aria-hidden="true">
              {QUIZ_OPTIONS.map((o) => (
                <span
                  key={o.value}
                  className={`quiz-preview__btn quiz-preview__btn--${o.value}`}
          ...
          {c.mod === "fakten" && (
            <div className="fakten-preview" aria-hidden="true">
              {faktenCards.map((fc) => (
                <span
                  key={fc.verdict}
                  className={`fk-mini fk-mini--${fc.verdict}`}
                  style={{ backgroundImage: fc.gradient }}
                >
                  <span className="fk-mini__statement">{fc.text}</span>
                  <svg
                    className="fk-mini__arrow"
                    viewBox={fc.arrow.viewBox}
                    fill="none"
                    preserveAspectRatio="xMidYMid meet"
                    dangerouslySetInnerHTML={{ __html: fc.arrow.inner }}
                  />
                </span>
              ))}
            </div>
            ...
            <div className="daten-preview" aria-hidden="true">
              <div className="daten-chart">
                ...
                {DATEN_BARS.map((b, i) => (
                  ...
                  >
                    <span
                      className="daten-bar__wash"
                      style={{ width: `${b.idle}%`, background: b.accent }}
                    />
                    <span
                      className="daten-bar__circle"
                      style={{ left: `${b.idle}%`, background: b.accent }}
                    >
                      {b.idle}
                    </span>
                  </div>
