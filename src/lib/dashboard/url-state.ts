...
const ALL_VERDICTS: CorrectnessClass[] = [
  "richtig",
  "eher_richtig",
  "eher_falsch",
  "falsch",
...
const ALL_SOURCE_METRICS: SourceMetricType[] = [
  "search",
  "perception",
  "trust",
  "prevention",
];
const ALL_SOURCE_GROUPS: SourceGroupId[] = [
  "adults",
  "minors",
  "consumers",
  "young_adults",
...
const ALL_QUIZ_THEME_SLUGS: QuizThemeSlug[] = [
...
const ALL_STRIPS_SORT_AXES: StripsSortAxis[] = [
  ...ALL_INDICATORS,
  ...ALL_GROUP_IDS,
] as StripsSortAxis[];
...
export function stateToUrl(state: Partial<AppState>): string {
  ...
  if (state.lang && state.lang !== DEFAULTS.lang)
  ...
  if (
    state.groupIds &&
    JSON.stringify(state.groupIds) !== JSON.stringify(DEFAULTS.groupIds)
  ) {
  ...
  if (state.spannweiteSortColumn)
    params.set("spcol", state.spannweiteSortColumn);

  if (
    state.sourcesSpannweiteSort &&
    state.sourcesSpannweiteSort !== DEFAULTS.sourcesSpannweiteSort
  )
  ...
  if (
    state.sourcesSpannweiteExpanded &&
    state.sourcesSpannweiteExpanded.length > 0
  )
  ...
  if (state.scatterX && state.scatterX !== DEFAULTS.scatterX)
    params.set("sx", state.scatterX);
  if (state.scatterY && state.scatterY !== DEFAULTS.scatterY)
    params.set("sy", state.scatterY);
  ...
  if (
    state.sourcesStripsMode &&
    state.sourcesStripsMode !== DEFAULTS.sourcesStripsMode
  )
...
export function urlToState(): Partial<AppState> {
  ...
  if (groupSingular) {
    const gid =
      ...
      (ALL_GROUP_IDS.includes(groupSingular as GroupId)
        ? (groupSingular as GroupId)
        : null);
  ...
  if (rawInd) {
    const ind =
      ...
      (ALL_INDICATORS.includes(rawInd as Indicator)
        ? (rawInd as Indicator)
        : null);
  ...
  if (
    s2sort &&
    ALL_SOURCES_SPANNWEITE_SORTS.includes(s2sort as SourcesSpannweiteSort)
  ) {
  ...
  if (ALL_INDICATORS.includes(sx as Indicator))
    state.scatterX = sx as Indicator;

  const sy = params.get("sy");
  if (ALL_INDICATORS.includes(sy as Indicator))
    state.scatterY = sy as Indicator;

  const sm = params.get("sm");
  if (ALL_SOURCE_METRICS.includes(sm as SourceMetricType))
    state.sourceMetric = sm as SourceMetricType;

  const sg = params.get("sg");
  if (ALL_SOURCE_GROUPS.includes(sg as SourceGroupId))
    state.sourceGroup = sg as SourceGroupId;
  ...
  if (ALL_STRIPS_MODES.includes(stm as StripsMode))
    state.stripsMode = stm as StripsMode;
  ...
  if (ALL_STRIPS_DIRS.includes(std as StripsSortDir))
    state.stripsSortDir = std as StripsSortDir;
  ...
  if (stt) {
    state.stripsThemeFilter = stt
      .split(",")
      .filter((s) =>
        ALL_QUIZ_THEME_SLUGS.includes(s as QuizThemeSlug),
      ) as QuizThemeSlug[];
  }
