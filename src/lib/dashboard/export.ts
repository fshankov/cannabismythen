...
/* ========================================================================
   Informationswege (information-channels) export. Mirrors the myth CSV but
   in long format (one row per channel × group), reading the same
   information-sources.json the live dashboard renders. The matching Excel
   workbook is generated by scripts/generate-informationswege-xlsx.py.
   ===================================================================== */
 ...
 *  channel × group, with the four indicator columns. Blank cell = the
 *  channel was not measured for that indicator. */
export function exportSourcesCSV(data: InformationSourcesData): string {
  const groupName = new Map(data.sourceGroups.map((g) => [g.id, g.name]));
  const catName = new Map(data.sourceCategories.map((c) => [c.id, c.name]));
