...
export function symbolDefs(): string {
  ...
  return [
    sym(
      "strips-arrow-richtig",
      "rotate(180 12 12)",
      full(VERDICT_FG.richtig, VERDICT_SHADOW.richtig),
    ),
    sym(
      "strips-arrow-eher_richtig",
      "rotate(-135 12 12)",
      full(VERDICT_FG.eher_richtig, VERDICT_SHADOW.eher_richtig),
    ),
    sym(
      "strips-arrow-eher_falsch",
      "rotate(45 12 12)",
      full(VERDICT_FG.eher_falsch, VERDICT_SHADOW.eher_falsch),
    ),
    sym(
      "strips-arrow-falsch",
      "",
      full(VERDICT_FG.falsch, VERDICT_SHADOW.falsch),
    ),
    sym(
      "strips-arrow-keine_aussage_moeglich",
      "",
      lineOnly(VERDICT_SHADOW.keine_aussage_moeglich),
    ),
  ].join("");
}
...
export function renderSpannweiteSvg(opts: RenderOpts): SVGSVGElement {
  ...
  parts.push(
    `<line x1="0" y1="${axisY}" x2="${TOTAL_W}" y2="${axisY}" stroke="#f1f5f9"/>`,
  );
  for (let i = 0; i < colCount; i++) {
    const cellX = LABEL_COL_W + i * colW;
