...
export function VizPeopleVoices({ active }: Props) {
  ...
  useEffect(() => {
    ...
    // Collision is weak so bubbles can overlap — that's the visual point: lots
    // of statements in people's heads, packed together.
    const sim = d3
      .forceSimulation<SimNode>(nodes)
      .force('center', d3.forceCenter(w / 2, h / 2).strength(0.02))
      .force(
        'collide',
        d3.forceCollide<SimNode>().radius((n) => n.size * 0.55).strength(0.45),
      )
  ...
  return (
    <div className="viz" style={{ width: '100%' }}>
      ...
      >
        {VOICES.map((v) => {
          const fontSize = voiceFontSize(v.weight);
          const opacity = 0.45 + v.weight * 0.55;
